...
Активний код: `apps/native`. Поточний checkpoint — **0.9.3+20**. «Мій портфель» тепер охоплює factual purchase/sale/redemption/coupon lifecycle, per-ISIN ledger, закриті позиції, factual cash summary та явний non-destructive legacy migration wizard поверх локального encrypted vault.
...
- **«Мій портфель»**: локальний encrypted portfolio, factual purchase/sale/redemption/coupon, per-ISIN ledger, closed positions, factual cash summary та explicit legacy migration wizard;
...
17. **DONE** — persistent «Економічний пульс» + first encrypted «Мій портфель» flow у v0.9.2+19; 18. **DONE** — sale/redemption/history + migration wizard; 19. **DONE** — coupon + per-ISIN ledger; 20. **DONE** — factual cash summary + closed positions; 21. **DOING** — v0.9.3+20 release checkpoint.
...
У v0.9.3 user-facing portfolio працює поверх перевіреної encrypted-vault foundation: create/open/lock, factual lifecycle, ledger, cash summary та explicit migration wizard. Legacy `sets/*.json` **не мігруються автоматично під час оновлення**; wizard копіює підтримувані приватні метадані до encrypted vault, але source JSON навмисно лишаються на місці й не видаляються автоматично. Не використовуйте legacy workspace для ключів підпису, KYC-документів чи інших секретів.
