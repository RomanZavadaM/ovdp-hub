...
**v0.9.3+20**은 사용자용 실제 포트폴리오 흐름을 확장합니다. 매수 로트에 명시적으로 배분하는 매도, 실제 상환과 쿠폰, 전체 이력, ISIN별 ledger, 종료된 포지션, 통화별 실제 현금 요약, 명시적 비파괴 legacy migration wizard를 포함합니다. 알 수 없는 수수료는 0으로 처리하지 않으며 시장가치를 실제 현금 결과로 표시하지 않습니다.
...
활성 코드는 `apps/native`에 있습니다. 현재 체크포인트는 **0.9.3+20**입니다. v0.9.2의 경제 지표/암호화 포트폴리오 진입점에 이후 통합된 실제 매도·상환·쿠폰·이력·리포팅·migration 흐름이 포함됩니다.
...
11. **DONE** — encrypted-vault/private-domain/비파괴 migration 기반; 12. **DONE** — v0.9.2+19 체크포인트; 13. **DONE** — 매도/상환/이력 + migration wizard; 14. **DONE** — 쿠폰 + ISIN별 ledger; 15. **DONE** — 실제 현금 요약 + 종료 포지션; 16. **DOING** — v0.9.3+20 release checkpoint.
...
선택 기능에서 사용하는 legacy workspace JSON은 **v0.9.3을 설치하는 것만으로 자동 이전되지 않습니다**. 명시적 migration wizard가 지원되는 비공개 메타데이터를 암호화 vault로 복사하고 암호화 복사본을 확인하지만, 원본 `sets/*.json`은 그대로 남으며 자동 삭제되지 않습니다. Legacy workspace에는 서명 키, KYC 문서 또는 기타 비밀정보를 저장하지 마십시오.
