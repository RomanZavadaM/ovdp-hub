...
**v0.9.3+20** vervollständigt den sichtbaren faktischen Portfolio-Lifecycle: Verkäufe mit expliziter Lot-Zuordnung, faktische Rückzahlungen und Kupons, Historie, ISIN-Ledger, geschlossene Positionen, faktische Cash-Übersicht je Währung und einen expliziten nicht-destruktiven Legacy-Migrationsassistenten. Unbekannte Gebühren bleiben unbekannt; Marktwert wird nicht als faktisches Cash-Ergebnis dargestellt.
...
Aktiver Code: `apps/native`. Der aktuelle Checkpoint ist **0.9.3+20**. Er enthält den Wirtschaftspuls und Portfolio-Einstieg aus v0.9.2 sowie die danach integrierten faktischen Verkauf/Rückzahlung/Kupon/Historie/Reporting- und Migrationsflüsse.
...
11. **DONE** — Encrypted-Vault/Private-Domain/nicht-destruktive Migration; 12. **DONE** — v0.9.2+19 Checkpoint; 13. **DONE** — Verkauf/Rückzahlung/Historie + Migration; 14. **DONE** — Kupon + ISIN-Ledger; 15. **DONE** — faktische Cash-Übersicht + geschlossene Positionen; 16. **DOING** — v0.9.3+20 Release-Checkpoint.
...
Legacy-Workspace-JSON wird **durch die Installation von v0.9.3 nicht automatisch migriert**. Der explizite Assistent kopiert unterstützte private Metadaten in den verschlüsselten Vault und prüft die verschlüsselte Kopie; die Quell-`sets/*.json` bleiben erhalten und werden nie automatisch gelöscht. Keine Signaturschlüssel, KYC-Dokumente oder andere Secrets im Legacy-Workspace speichern.
